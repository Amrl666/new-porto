...
export default function Home() {
  const {
    ...
    error: userInfoError,
  } = useSWR<UserInfo>(getUserInfo, fetcher);
  ...
  if (isError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 bg-background text-foreground">
        ...
        <p className="text-sm text-muted-foreground">Periksa koneksi atau isi .env Sanity.</p>
      </div>
    );
  }
  ...
  if (!userInfo || !experience || !technologies) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3 bg-background text-foreground">
        ...
        <p className="text-sm text-muted-foreground">Pastikan Sanity terisi atau jalankan seeding.</p>
      </div>
    );
  }
